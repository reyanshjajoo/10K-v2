...
void left7()
{
  ...
  chassis.pid_odom_set({{22, -20}, fwd, DRIVE_SPEED - 30});
  chassis.pid_wait_until(20_in);
  matchload.set_value(true);
  chassis.pid_wait_quick();
  chassis.pid_turn_set({54, -47}, fwd, TURN_SPEED);
  chassis.pid_wait_quick();
  chassis.pid_odom_set({{54, -47}, fwd, DRIVE_SPEED});
  chassis.pid_wait_quick();
  chassis.pid_turn_set(90_deg, TURN_SPEED);
  chassis.pid_wait_quick();
  chassis.pid_drive_set(12_in, MATCHLOAD_SPEED, true);
  chassis.pid_wait_quick();
  pros::delay(200);
...
void left_3_4()
{
  ...
  chassis.pid_turn_exit_condition_set(80_ms, 3_deg, 200_ms, 7_deg, 400_ms, 500_ms);
  chassis.pid_drive_set(13_in, 60); // intake matchload
  chassis.pid_wait();
  pros::delay(350);                             // matchlaod delay
  ...
  intakeState = IntakeState::intake;
  chassis.pid_odom_set({{-25, 20}, fwd, DRIVE_SPEED_AWP - 10}); // go to 3 ball
  chassis.pid_wait_quick();
  chassis.pid_drive_set(5_in, DRIVE_SPEED - 20, true);      // adjust
  chassis.pid_wait_quick();
  pros::delay(900);
  //matchload.set_value(true);
  chassis.pid_wait();
  ...
  pros::delay(1500);
  intakeState = IntakeState::intake;
  chassis.pid_drive_set(36.1_in, DRIVE_SPEED, true);
  chassis.pid_wait_quick();
  chassis.pid_turn_set(90_deg, TURN_SPEED);
  chassis.pid_wait_quick();
  horn.set_value(true);
...
void awp()
{
  ...
  chassis.pid_drive_set(13_in, 60); // intake matchload
  chassis.pid_wait();
  pros::delay(250);                             // matchlaod delay
  ...
  matchload.set_value(false);
  chassis.pid_swing_set(ez::LEFT_SWING, 215_deg, SWING_SPEED, -60, ez::cw);
  pros::delay(900);
  ...
  pros::delay(400);
  chassis.pid_drive_set(-18_in, DRIVE_SPEED-10, true);
  pros::delay(450); // tuen
  ...
  chassis.pid_turn_exit_condition_set(80_ms, 3_deg, 200_ms, 7_deg, 400_ms, 500_ms);
  chassis.pid_drive_set(16_in, 55); // matchload
...
void skills()
{
  ...
  chassis.pid_drive_set(14_in, 127); // back out and turn 180 to reset
  chassis.pid_wait();
  chassis.pid_drive_set(-25_in, 127); // back out and turn 180 to reset
  chassis.pid_wait();
  chassis.pid_drive_set(11_in, 60); // back out and turn 180 to reset
  chassis.pid_wait();
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{-10, 0}, rev, DRIVE_SPEED});
  chassis.pid_wait();
