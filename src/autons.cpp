...
void left_3_4()
{
  ...
  chassis.pid_wait_quick_chain();
  chassis.pid_drive_set(8.7_in, 55); // intake matchload
  pros::delay(800);                             // matchlaod delay
  chassis.pid_drive_set(-33_in, DRIVE_SPEED); // back from matchload
  pros::delay(900);
  intakeState = IntakeState::highGoal;
  matchload.set_value(false);
  ...
  chassis.pid_wait_quick();
  chassis.pid_drive_set(-22.25_in, DRIVE_SPEED-10, true);
  pros::delay(900);
...
void left_3_4_new()
{
  ...
  chassis.pid_wait_quick();
  chassis.pid_drive_set(-18_in, DRIVE_SPEED-30, true);
  chassis.pid_wait();
  chassis.drive_set(-30, -30);
  intakeState = IntakeState::midGoal;           // score mid goal
  pros::delay(1000);
...
void left4()
{
  ...
  wing.set_value(true);
  chassis.odom_xyt_set(-48_in, 11_in, 90_deg);
...
void left7()
{
  ...
  chassis.pid_wait_until(14_in);
  matchload.set_value(true);
  chassis.pid_turn_set({-50, 48}, fwd, 127);
  pros::delay(450);
  chassis.pid_odom_set({{-50, 48}, fwd, 127});
  chassis.pid_wait();
...
void right_5ball_rush()
{
  ...
  matchload.set_value(true);
  pros::delay(100);
  chassis.pid_turn_set({5, 47}, fwd, TURN_SPEED);
...
void awp()
{
  ...
  pros::delay(700); // score mid goal
  intakeState = IntakeState::outtakeMid;
  pros::delay(150);
  //matchload.set_value(false);
  intakeState = IntakeState::intake;
  chassis.pid_turn_set(140, TURN_SPEED);
  chassis.pid_drive_set(52_in, DRIVE_SPEED, true);//to matchload 
  pros::delay(1450);
  chassis.pid_turn_set(90_deg, TURN_SPEED);
  pros::delay(350); 
  chassis.pid_drive_set(19_in, 55, true); // matchload
  pros::delay(1300);                           // matchlaod delay
...
void skills()
{
  ...
  intakeState = IntakeState::intake;
  chassis.pid_drive_set(5_in, DRIVE_SPEED-40);
  pros::delay(400);
  ...
  chassis.odom_xyt_set(-45, distanceDiff, 270);//:)
  intakeState = IntakeState::intake;
  
  chassis.pid_drive_set(-5_in, DRIVE_SPEED);
  chassis.pid_wait();
  chassis.pid_odom_set({{-16, 0}, rev, DRIVE_SPEED});
  chassis.pid_wait();
  chassis.pid_turn_set(0, TURN_SPEED);
  chassis.pid_wait();
  // chassis.pid_drive_set(20_in, 40); // intake single blue block
  chassis.pid_odom_set({{-16, 20}, fwd, 40});
  chassis.pid_wait();
  chassis.pid_turn_set(-45_deg, TURN_SPEED-25);
  chassis.pid_wait();
  intakeState = IntakeState::idle;
  ...
  chassis.pid_drive_set(5_in, DRIVE_SPEED);
  chassis.pid_wait();
  chassis.pid_turn_set({61, 28}, fwd, TURN_SPEED);
  chassis.pid_odom_ptp_set({{59, 28}, fwd, DRIVE_SPEED});
  chassis.pid_wait();
  chassis.pid_turn_set(135_deg, TURN_SPEED);
  chassis.pid_wait();
  ...
  pros::delay(650);
  chassis.pid_drive_set(-17_in, 40);//reset on park zone 
  chassis.pid_wait();
  matchload.set_value(false);
  chassis.pid_drive_set(5_in, DRIVE_SPEED);
  chassis.pid_wait();
